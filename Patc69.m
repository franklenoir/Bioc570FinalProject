function [cellfinal,results] = Patc69()

%%
%img1
    
imname = strcat('./PATC69/1.tif');
...
%cells = {'MeanConnections','MinCentroidDist','MeanCentroidCluster','PercentsCentsInClusters'};
con = bwconncomp(mask);
cells(6,1:5) = {con.Connectivity/o,mean(mindist),max(mean(measurements.meancents)),sum(measurements.nuclei)/o2,'Epithelial'};

resultmes= vertcat(resultmes,measurements);

%%
imname = strcat('./PATC69/7.tif');
reader1 = bfGetReader(imname);
iplane = reader1.getIndex(1-1,1-1,1-1)+1;
img = bfGetPlane(reader1,iplane);
img2 = im2double(img);
...
%cells = {'MeanConnections','MinCentroidDist','MeanCentroidCluster','PercentsCentsInClusters'};
con = bwconncomp(mask);
cells(7,1:5) = {con.Connectivity/o,mean(mindist),max(mean(measurements.meancents)),sum(measurements.nuclei)/o2,'Epithelial'};
